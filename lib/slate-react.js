...
function AfterPlugin() {
  ...
  function onInput(event, editor, next) {
    ...
    // prettier-ignore
    console.log('    flush selAfterOnInput:', JSON.stringify(editor.value.selection.toJSON()));
    ...
    // prettier-ignore
    console.log('    flush selAfterReconci:', JSON.stringify(editor.value.selection.toJSON()));
    // prettier-ignore
    console.log('    editor: len: ' + editor.value.document.text.length + ' selSlate: ' + editor.value.selection.anchor.offset + ' selNative: ' + window.getSelection().anchorOffset + ' document: ' + JSON.stringify(editor.value.document.toJSON()));
  ...
  function onSelect(event, editor, next) {
    ...
    var selection = editor.findSelection(domSelection);
    // prettier-ignore
    if (domSelection && editor.value.selection.anchor && selection && selection.anchor) console.log('!! onSelect domOffset:' + domSelection.anchorOffset + ' oldSlate: ' + editor.value.selection.anchor.offset + ' newSlate: ' + selection.anchor.offset);
...
function BeforePlugin() {
  ...
  function onBeforeInput(event, editor, next) {
    console.log('!! onBeforeInput with data:' + event.data + ' inputType:' + event.inputType + ' has2:' + slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2 + ' isSynthetic:' + !!event.nativeEvent);
  ...
  function onKeyDown(event, editor, next) {
    if (editor.readOnly) return;
    console.log('!! onKeyDown isComposing:' + isComposing + ' hasOp:' + !!editor.controller.tmp.nextNativeOperation + ' mods:' + (event.ctrlKey ? 'ctrl-' : '') + (event.altKey ? 'alt-' : '') + (event.shiftKey ? 'shift-' : '') + (event.metaKey ? 'meta-' : '') + ' key:' + event.key + ' which:' + event.which);
  ...
  function syncDomToSlateAst(editor) {
    ...
    console.log('    slateDomSpan: ' + slateDomSpan.textContent + ' ' + slateDomSpan.textContent.length);
    console.log('    slateAstNode: ' + slateAstNode.text + ' ' + slateAstNode.text.length);
      ...
      for (var _iterator = allChildTextNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        if (isStringNode || isZeroWidth) {
          ...
          if (isStringNode && hasZeroWidthChars || isZeroWidth && stringNode.textContent !== '\uFEFF') {
            ...
            console.log('    REPLACING ' + stringNode.childNodes.length);
