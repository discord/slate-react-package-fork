...
function BeforePlugin() {
  ...
  function onCompositionEnd(event, editor, next) {
    if (!window.NO_SLATE_LOGGING) console.log('!! onCompositionEnd isComposing:' + isComposing);
    isUserActionPerformed = true;
    isComposing = false;
  ...
  function onCompositionStart(event, editor, next) {
    if (!window.NO_SLATE_LOGGING) console.log('!! onCompositionStart isComposing:' + isComposing + ' isCollapsed:' + editor.value.selection.isCollapsed);
    isComposing = true;
  ...
  function onInput(event, editor, next) {
    ...
    if (isComposing) {
      // Safari is broken :(
      if (slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2) return;
  ...
  function onSelect(event, editor, next) {
    if (isCopying) return;
    if (editor.readOnly) return;
    if (isComposing && slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2) return;
  ...
  function syncDomToSlateAst(editor) {
    ...
    var _loop = function _loop(node) {
      sanitizeDomOnError(editor, node, function () {
    ...
    try {
      for (var _iterator = nextNativeOperation[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  function syncNodeToSlateAst(editor, slateDomSpan) {
    ...
    if (!window.NO_SLATE_LOGGING) console.log('    editor: len: ' + editor.value.document.text.length + ' selSlate: ' + editor.value.selection.anchor.offset + ' document: ' + JSON.stringify(editor.value.document.toJSON()));
      ...
      for (var _iterator2 = allChildTextNodes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        if (isStringNode || isZeroWidth) {
          ...
          if (isStringNode && hasZeroWidthChars || isZeroWidth && stringNode.textContent !== '\uFEFF') {
            ...
            try {
              for (var _iterator3 = stringNode.childNodes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
  ...
  function saveCurrentNativeNode(editor) {
    ...
    addCurrentlySelectedKeyNode(editor, editor.controller.tmp.nextNativeOperation);
  }
  ...
  function addCurrentlySelectedKeyNode(editor, keyNodes) {
    ...
    var currentSpan = safelyGetParentKeyNode(window.getSelection().anchorNode);
