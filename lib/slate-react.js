...
var TextString = function (_React$Component) {
  inherits(TextString, _React$Component);

  function TextString(props) {
    classCallCheck(this, TextString);

    var _this = possibleConstructorReturn(this, (TextString.__proto__ || Object.getPrototypeOf(TextString)).call(this, props));
  ...
  createClass(TextString, [{
    ...
    value: function shouldComponentUpdate(nextProps) {
      ...
      var reactTextContent = nextProps.children;
      ...
      if (nextProps.isZeroWidth && domTextContent.replace(/[\uFEFF]/g, '') !== '') return true;
    ...
    value: function render() {
      var _props = this.props,
          isZeroWidth = _props.isZeroWidth,
          isLineBreak = _props.isLineBreak,
      ...
      if (isZeroWidth) {
        ...
        return React.createElement(
          ...
          _extends({
            ...
            ref: this.ref
          }, (_ref = {}, defineProperty(_ref, DATA_ATTRS.ZERO_WIDTH, isLineBreak ? 'n' : 'z'), defineProperty(_ref, DATA_ATTRS.LENGTH, length || 0), _ref)),
          '\uFEFF',
...
var Leaf = function Leaf(props) {
  ...
  if (editor.query('isVoid', parent)) {
    ...
    children = React.createElement(TextString, { isZeroWidth: true, length: parent.text.length });
  } else if (text === '' && parent.object === 'block' && parent.text === '' && parent.nodes.last() === node) {
    // COMPAT: If this is the last text node in an empty block, render a zero-
    // width space that will convert into a line break when copying and pasting
    // to support expected plain text.
    children = React.createElement(TextString, { isZeroWidth: true, isLineBreak: true });
  } else if (text === '') {
    // COMPAT: If the text is empty, it's because it's on the edge of an inline
    // node, so we render a zero-width space so that the selection can be
    // inserted next to it still.
    children = React.createElement(TextString, { isZeroWidth: true });
  } else {
...
var Content = function (_React$Component) {
  ...
  createClass(Content, [{
    ...
    value: function componentDidMount() {
      ...
      if (slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2 && this.ref.current) {
...
function CommandsPlugin() {
  ...
  function reconcileNode(editor, node) {
    ...
    if (text.replace(/[\uFEFF]/g, '') === domText.replace(/[\uFEFF]/g, '')) return;
    ...
    editor.insertTextAtRange(entire, domText.replace(/[\uFEFF]/g, ''), node.marks);
    return;
  }
  ...
  function reconcileDOMNode(editor, domNode) {
    try {
      ...
      // hopefully.
      var _domElement = domNode.hasAttribute && domNode.hasAttribute('data-key') ? domNode : domNode.parentElement.closest('[data-key]');
      if (_domElement == null) return;

      var slateNode = editor.value.document.getChild(domNode.getAttribute('data-key'));
      ...
      if (!window.NO_SLATE_LOGGING) console.log('    replacing entire line');
      editor.replaceNodeByKey(blockNode.key, slate.Block.create(blockNode.toJSON()));
    }
  }
...
function AfterPlugin() {
  ...
  function onInput(event, editor, next) {
    ...
    // prettier-ignore
    if (!window.NO_SLATE_LOGGING) console.log('    flush selAfterOnInput:', JSON.stringify(editor.value.selection.toJSON()));
    ...
    // prettier-ignore
    if (!window.NO_SLATE_LOGGING) console.log('    flush selAfterReconci:', JSON.stringify(editor.value.selection.toJSON()));
    // prettier-ignore
    if (!window.NO_SLATE_LOGGING) console.log('    editor: len: ' + editor.value.document.text.length + ' selSlate: ' + editor.value.selection.anchor.offset + ' selNative: ' + window.getSelection().anchorOffset + ' document: ' + JSON.stringify(editor.value.document.toJSON()));
  ...
  function onSelect(event, editor, next) {
    ...
    var selection = editor.findSelection(domSelection);
    // prettier-ignore
    if (!window.NO_SLATE_LOGGING && domSelection && editor.value.selection.anchor && selection && selection.anchor) console.log('!! onSelect domOffset:' + domSelection.anchorOffset + ' oldSlate: ' + editor.value.selection.anchor.offset + ' newSlate: ' + selection.anchor.offset);
...
function BeforePlugin() {
  ...
  function onBeforeInput(event, editor, next) {
    if (!window.NO_SLATE_LOGGING) console.log('!! onBeforeInput with data:' + event.data + ' inputType:' + event.inputType + ' has2:' + slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2 + ' isSynthetic:' + !!event.nativeEvent);
      ...
      if (isCollapsed && !hasNewLines) {
        ...
        if (!hasNewLines) {
          ...
          chunks.map(function (text, i) {
            if (text.length !== 0) {
              if (!window.NO_SLATE_LOGGING) console.log('    insert: |' + text + '|');
              editor.insertText(text, null, false);
            }

            if (i !== chunks.length - 1) {
              if (!window.NO_SLATE_LOGGING) console.log('    inserting line break');
              editor.splitBlock();
            }
          });
        }
  ...
  function onCompositionStart(event, editor, next) {
    if (!window.NO_SLATE_LOGGING) console.log('!! onCompositionStart isCollapsed:' + editor.value.selection.isCollapsed);
    isComposing = true;
  ...
  function onInput(event, editor, next) {
    if (!window.NO_SLATE_LOGGING) console.log('!! onInput isComposing:' + isComposing + ' hasOp:' + !!editor.controller.tmp.nextNativeOperation);
    if (isComposing) return;
  ...
  function onKeyDown(event, editor, next) {
    if (editor.readOnly) return;
    if (!window.NO_SLATE_LOGGING) console.log('!! onKeyDown isComposing:' + isComposing + ' hasOp:' + !!editor.controller.tmp.nextNativeOperation + ' mods:' + (event.ctrlKey ? 'ctrl-' : '') + (event.altKey ? 'alt-' : '') + (event.shiftKey ? 'shift-' : '') + (event.metaKey ? 'meta-' : '') + ' key:' + event.key + ' which:' + event.which);
  ...
  function syncDomToSlateAst(editor) {
    if (!window.NO_SLATE_LOGGING) console.log('!! syncDomToSlateAst');
    var nextNativeOperation = editor.controller.tmp.nextNativeOperation;

    if (!nextNativeOperation) return false;
    editor.controller.tmp.nextNativeOperation = null;

    var oldSlateSelection = nextNativeOperation.slateSelection,
        slateDomSpan = nextNativeOperation.slateDomSpan;

    var _window$getSelection = window.getSelection(),
        textNode = _window$getSelection.anchorNode,
        currentOffset = _window$getSelection.anchorOffset;
    ...
    var currentSlateDomSpan = textNode.parentElement.closest(SELECTORS.KEY);
    if (currentSlateDomSpan == null) throw Error('YOWIE WOWIE: could not find slate span');
    if (slateDomSpan !== currentSlateDomSpan) throw Error('YOWIE WOWIE: slate span node mismatch');

    var key = oldSlateSelection.anchor.key;
    ...
    if (!window.NO_SLATE_LOGGING) console.log('    textNode: ' + textNode.textContent + ' ' + textNode.textContent.length);
    if (!window.NO_SLATE_LOGGING) console.log('    slateDomSpan: ' + slateDomSpan.textContent + ' ' + slateDomSpan.textContent.length);
    if (!window.NO_SLATE_LOGGING) console.log('    slateAstNode: ' + slateAstNode.text + ' ' + slateAstNode.text.length);
    if (!window.NO_SLATE_LOGGING) console.log('    flush selBeforeInsert:', JSON.stringify(editor.value.selection.toJSON()));
    if (!window.NO_SLATE_LOGGING) console.log('    editor: len: ' + editor.value.document.text.length + ' selSlate: ' + editor.value.selection.anchor.offset + ' selNative: ' + currentOffset + ' document: ' + JSON.stringify(editor.value.document.toJSON()));
    ...
    var newTextContent = slateDomSpan.textContent.replace(/[\uFEFF]/g, '');
    syncTextToAst(editor, slateAstNode, path, newTextContent);

    if (!window.NO_SLATE_LOGGING) console.log('    flush selAfterInsert :', JSON.stringify(editor.value.selection.toJSON()));
    if (!window.NO_SLATE_LOGGING) console.log('    editor: len: ' + editor.value.document.text.length + ' selSlate: ' + editor.value.selection.anchor.offset + ' selNative: ' + currentOffset + ' document: ' + JSON.stringify(editor.value.document.toJSON()));
    ...
    if (!window.NO_SLATE_LOGGING) console.log('    flush selAfterSelect1:', JSON.stringify(editor.value.selection.toJSON()));
    ...
    if (!window.NO_SLATE_LOGGING) console.log('    flush selAfterSelect2:', JSON.stringify(editor.value.selection.toJSON()));
  ...
  function syncTextToAst(editor, slateAstNode, path, newText) {
    ...
    if (numStartingCharsSame + numEndingCharsSame !== oldText.length) {
      ...
      editor.insertTextAtRange(slate.Range.create({
  ...
  function setSlateSelection(editor, path, key, offset) {
    ...
    if (!selection.isCollapsed || selection.anchor.key !== key || selection.anchor.offset !== offset) {
      ...
      editor.select(slate.Range.create({ anchor: point, focus: point }));
    }
  }
  ...
  function sanitizeZeroWidthSpaces(editor, slateDomSpan) {
    ...
    try {
      for (var _iterator = allChildTextNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var stringNode = _step.value;
        ...
        if (isStringNode || isZeroWidth) {
          ...
          if (isStringNode && hasZeroWidthChars || isZeroWidth && stringNode.textContent !== '\uFEFF') {
            ...
            try {
              for (var _iterator2 = stringNode.childNodes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            if (!window.NO_SLATE_LOGGING) console.log('    REPLACING ' + stringNode.childNodes.length);
            ...
            if (stringNode.childNodes.length === 1) {
              stringNode.childNodes[0].textContent = stringNode.childNodes[0].textContent.replace(/[\uFEFF]/g, '');
            } else {
              stringNode.textContent = stringNode.textContent.replace(/[\uFEFF]/g, '');
            }
  ...
  function saveCurrentNativeNode(editor) {
    ...
    editor.controller.tmp.nextNativeOperation = {
      ...
      slateDomSpan: window.getSelection().anchorNode.parentElement.closest(SELECTORS.KEY)
    };
  }
...
function ReactPlugin() {
  ...
  var placeholderPlugin = slateDevEnvironment.IS_ANDROID || placeholder === null ? null : PlaceholderPlugin({
