    ...
    value: function render() {
      ...
      if (isZeroWidth) {
        ...
        return React.createElement(
          ...
          '\uFEFF',
          isLineBreak || isOnly ? React.createElement('br', null) : null
        );
      } else {
        return React.createElement(
          'span',
...
var Leaf = function Leaf(props) {
  ...
  var isOnly = leaves.size === 1;
  var children = void 0;

  if (editor.query('isVoid', parent)) {
    // COMPAT: Render text inside void nodes with a zero-width space.
    // So the node can contain selection but the text is not visible.
    children = React.createElement(TextString, { isOnly: isOnly, isZeroWidth: true, length: parent.text.length });
  } else if (text === '' && parent.object === 'block' && parent.text === '' && parent.nodes.last() === node) {
    // COMPAT: If this is the last text node in an empty block, render a zero-
    // width space that will convert into a line break when copying and pasting
    // to support expected plain text.
    children = React.createElement(TextString, { isOnly: isOnly, isZeroWidth: true, isLineBreak: true });
  } else if (text === '') {
    // COMPAT: If the text is empty, it's because it's on the edge of an inline
    // node, so we render a zero-width space so that the selection can be
    // inserted next to it still.
    children = React.createElement(TextString, { isOnly: isOnly, isZeroWidth: true });
  } else {
