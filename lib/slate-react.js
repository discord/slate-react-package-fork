...
var Content = function (_React$Component) {
  ...
  function Content() {
    ...
    return _ret = (_temp = (_this = possibleConstructorReturn(this, (_ref = Content.__proto__ || Object.getPrototypeOf(Content)).call.apply(_ref, [this].concat(args))), _this), _this.tmp = {
      ...
      if (_this.isComposing && slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2) return;
...
function sanitizeDomOnError(editor, domNode, fn) {
  try {
    ...
    if (selection != null && selection.isCollapsed && blockNode === slateNode && lineIndex >= 0 && domElement.tagName === 'DIV' && domElement.childNodes.length === 1 && domElement.childNodes[0] === selection.anchorNode) {
      console.log('    replacing entire line via dom text');

      editor.replaceNodeByKey(blockNode.key, slate.Block.create({
        type: 'line',
        nodes: [slate.Text.create({ text: domElement.textContent })]
      }));
...
function AfterPlugin() {
  ...
  function onKeyDown(event, editor, next) {
    ...
    if (Hotkeys.isDeleteBackward(event) && !slateDevEnvironment.IS_IOS) {
      return isRtl ? editor.deleteCharForward() : editor.deleteCharBackward();
    }

    if (Hotkeys.isDeleteForward(event) && !slateDevEnvironment.IS_IOS) {
      return isRtl ? editor.deleteCharBackward() : editor.deleteCharForward();
    }

    if (Hotkeys.isDeleteLineBackward(event)) {
      return isRtl ? editor.deleteLineForward() : editor.deleteLineBackward();
    }

    if (Hotkeys.isDeleteLineForward(event)) {
      return isRtl ? editor.deleteLineBackward() : editor.deleteLineForward();
    }

    if (Hotkeys.isDeleteWordBackward(event)) {
      return isRtl ? editor.deleteWordForward() : editor.deleteWordBackward();
    }

    if (Hotkeys.isDeleteWordForward(event)) {
      return isRtl ? editor.deleteWordBackward() : editor.deleteWordForward();
    }
...
function BeforePlugin() {
  ...
  function onBeforeInput(event, editor, next) {
    if (window.ENABLE_SLATE_LOGGING) console.log('!! onBeforeInput with data:' + event.data + ' inputType:' + event.inputType + ' has2:' + slateDevEnvironment.HAS_INPUT_EVENTS_LEVEL_2 + ' isSynthetic:' + !!event.nativeEvent);
